# Changelog
# 1.5 - Now clears manager field, grabs group memberships, wipes mobile outlook containers, removed mailbox forward,
    ...
    try {
        ...
        switch ($answer) {
            "Single" {
                ...
                Get-MgUserMemberOf -UserId $UserMail | ForEach-Object { Get-MgGroup -GroupId $_.Id } | Select-Object DisplayName | Export-Csv -Path "PATH\TO\CSV\$username.csv" -NoTypeInformation
                ...
                Move-ADObject -Identity (Get-ADUser -Filter { sAMAccountName -eq $UserSAM }).DistinguishedName -TargetPath "DISTINGUISHED OU"
            ...
            "Bulk" {
                ...
                if ($confirmation -eq "Y") {
                    ...
                    Import-Csv -Path "PATH\TO\CSVBulkOffboarding.csv" | ForEach-Object {
                        $Username = ($_.username)
                        $UserSAM = ($Username -replace '(?<=(.{20})).+')
                        $UserMail = ($Username + "@DOMAIN.com")
                        $Password = -join ((65..90) + (97..122) + (48..57) | Get-Random -Count 20 | ForEach-Object { [char]$_ })
