...
import-pssession $ExchangeSession
	...
	if ($ExchangeSession -eq $Null) {$ExchangeSession = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://CASSERVER2/PowerShell/
...
if ($RanAsEmail -eq $Null) {$RanAsEmail = "me@domain.com"} #This is there because I run my privileged stuff as an account other than my normal user account and my admin account doesn't have an email address
...
until (($ADUser -ne $null) -and $UserOK)
...
until (($ForwardADInfo -ne $null) -and $UserOK)
...
until (($FullAccessInfo -ne $null) -and $UserOK)
...
Write-Host " "
if ($ActiveSyncDevices -eq $Null) {Write-Host "There are no ActiveSync device memberships for $SAMAccountName."}
if ($ActiveSyncDevices -ne $Null) {Write-Host "There are ActiveSync device memberships for $SAMAccountName.  You will pick whether to remove them or send a wipe signal."}
...
if ($forwardto -eq $null) {write-host "You've chosen not to forward email"}
if ($forwardto -ne $null) {write-host "Email will be forwarded to $ForwardToCN"}
if ($fullaccessto -eq $null) {write-host "You've chosen to not grant anyone Full Mailbox Access permissions"}
if ($fullaccessto -ne $null) {write-host "Full Mailbox Access permissions will be granted to $FullAccessToCN"}
...
# Getting ActiveSync Device Information
if ($ActiveSyncDevices -ne $Null) {$ActiveSyncDevices | Out-File $ActiveSyncDevicesFile}
...
if ($ForwardToDN -ne $Null) {Write-Host " "}
if ($ForwardToDN -ne $Null) {Write-Host "Setting mail forwarding to $ForwardTo"}
if ($ForwardToDN -ne $Null) {Set-Mailbox -Identity $DN -ForwardingAddress $ForwardToDN -DeliverToMailboxAndForward:$false}

# Grant Full Access Permissions to a user
if ($FullAccessToDN -ne $Null) {Write-Host " "}
if ($FullAccessToDN -ne $Null) {Write-Host "Granting Full Mailbox Access to $FullAccessTo"} 
if ($FullAccessToDN -ne $Null) {Add-MailboxPermission -Identity $DN -user $FullAccessTo -AccessRights FullAccess -Confirm:$False | Out-Null} 
...
" " | Out-File $TermOutputFile -Append
if ($TermActiveSyncDevices -ne $Null) {$TermActiveSyncDevices | fl | Out-File $TermOutputFile -Append}
" " | Out-File $TermOutputFile -Append
