...
#ADDS THE GROUP Disabled Users TO THE DISABLED USER	
Add-AdGroupMember -identity "SG-Disabled Users" -Member $userinput

#SETS THE USERS TITLE,COMPANY/MANAGER TO DISABLED
set-aduser -identity $userinput -title "Disabled $Month$Day$Year"
...
Get-ADUser -Filter { samAccountName -like $userinput } | Move-ADObject –TargetPath "OU=Disabled,OU=Users,OU=LiteraMS,DC=literams,DC=net"
...
Set-MsolUserLicense -UserPrincipalName $userinput@litera.com -RemoveLicenses "litera:SPE_E3", "litera:"
...
$Mail.To = "ITHelp@litera.com"
$Mail.Subject = "[SR-$ticketnumber] "
$Mail.Body = @"
...
Changed Job Title to:  Disabled $Month$Day$Year
