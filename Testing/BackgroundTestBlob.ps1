...
Process {
    ...
    function Get-AzureBlobContent {
        ...
        try {   
            ...
            try {
                ...
                $StorageBlobContents = Get-AzStorageBlob -Container $ContainerName -Blob $BlobName -Context $StorageAccountContext -ErrorAction Stop
    ...
    $AzureStorageBlobContent = Get-AzureBlobContent -StorageAccountName $StorageAccountName -ContainerName $ContainerName -Blob $BlobName
    if ($null -ne $AzureStorageBlobContent) {
        ...
        if ($null -ne $DefaultWallpaperBlobFile) {
        ...
        if ($null -ne $WallpaperBlob4KFiles) {
