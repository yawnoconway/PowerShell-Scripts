...
$users = Get-ADUser -Filter * -Properties * -SearchBase $BaseOU | Where-Object {
    ...
    if ($_) {
        $userOU = $_.DistinguishedName -replace '^CN=[^,]+,'
        $ExcludeOUs -notcontains $userOU
    }
}
...
if ($users.Length -gt 0) { 
    foreach ($user in $users) {
        ...
        $userObj = [PSCustomObject]@{
            ...
            Profile_Description = $user.employeeType
            ...
            Practice_Area_Description = $user.extensionAttribute9
            ...
            Telephone_Number = $user.jobclass 
            ...
            Start_Date = (Get-Date $user.extensionAttribute11).ToString('yyyy-MM-dd')
    ...
    $path = 'C:\TEMP\Intellek\Internal\Import_Learners.csv'
